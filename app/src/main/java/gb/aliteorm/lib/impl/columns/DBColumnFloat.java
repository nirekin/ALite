...
/**
 * Database column to map <code>float</code> and <code>Float</code> values.
 *
 * @author Guillaume Barré
 * @since 1.0
 *
 */
public class DBColumnFloat extends DBColumnImpl{

	/**
	 * New column to map <code>float</code> and <code>Float</code> values
	 * 
	 ...
	 */
	protected DBColumnFloat(String tableName, Attribute a, Method getter) throws UnsupportedGetterException{
	...
	protected void defineSQL(StringBuilder strb) {
		...
		if(getDefaultValue() != null && getDefaultValue().trim().length() > 0){
			try{
				float f = Float.parseFloat(getDefaultValue());
				strb.append(" DEFAULT '" + f + "'");
			}catch(Exception e){
				throw new RuntimeException("Wrong the default value for float : " + a.getModelName() );
			}
		}
	}
	...
	public void readAttributed(StringBuilder strb, Object target){
		try {
			strb.append((float)a.getGetter().invoke(target));
		}catch(Exception e) {
			Log.e("bccore", "Error getter float :" + a.getModelName() );
		}
	}
